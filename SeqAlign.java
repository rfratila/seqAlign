...
public class SeqAlign {
	...
	public static void align(String s, String t, int a, int b){
		...
		for (int i = 0; i < sLength; i++){
			X0[i][0] = (i%3 == 0) ? i*a : posInfinity;
			X1[i][0] = (i%3 == 1) ? i*b : posInfinity;
			X2[i][0] = (i%3 == 2) ? i*b : posInfinity;
		}

		for (int j = 0; j < tLength; j++){
			Y0[0][j] = (j%3 == 0) ? j*a : posInfinity;
			Y1[0][j] = (j%3 == 1) ? j*b : posInfinity;
			Y2[0][j] = (j%3 == 2) ? j*b : posInfinity;
		}
